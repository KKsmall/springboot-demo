...
public class SecurityController {
    ...
    /**
     * 其实这里只要处理失败的逻辑就可以，验证认证shiro内部会拦截认证，认证成功会直接重定向到成功路径
     * @param request
     * @return
     */
    @ResponseBody
    @PostMapping("/login")
    public String login(HttpServletRequest request) { // <1> 判断是否已经登陆
        Subject subject = SecurityUtils.getSubject();
        if (subject.getPrincipal() != null) {
            return "你已经登陆账号：" + subject.getPrincipal();
        }

        // <2> 获得登陆失败的原因 登录失败shiro会往attribute里设置属性
